...
class ElementInformationController
{
    ...
    protected function makeRef($table, $ref)
    {
        ...
        $predicates = [
            'ref_table=' . $this->getDatabaseConnection()->fullQuoteStr($selectTable, 'sys_refindex'),
            'ref_uid=' . (int)$selectUid
        ];

        $backendUser = $this->getBackendUser();
        if (!$backendUser->isAdmin()) {
            $allowedSelectTables = GeneralUtility::trimExplode(',', $backendUser->groupData['tables_select']);
            $quotedTableList = $this->getDatabaseConnection()->fullQuoteArray($allowedSelectTables, $table);
            $predicates[] = 'tablename IN(' . implode(',', $quotedTableList) . ')';
        }

        $rows = $this->getDatabaseConnection()->exec_SELECTgetRows(
            '*',
            'sys_refindex',
            implode(' AND ', $predicates) . ' AND deleted=0'
        );
    ...
    protected function makeRefFrom($table, $ref)
    {
        $lang = $this->getLanguageService();
        $predicates = [
            'tablename=' . $this->getDatabaseConnection()->fullQuoteStr($table, 'sys_refindex'),
            'recuid=' . (int)$ref
        ];
        ...
        if (!$backendUser->isAdmin()) {
            $allowedSelectTables = GeneralUtility::trimExplode(',', $backendUser->groupData['tables_select']);
            $quotedTableList = $this->getDatabaseConnection()->fullQuoteArray($allowedSelectTables, $table);
            $predicates[] = 'ref_table IN(' . implode(',', $quotedTableList) . ')';
        }
     ...
     * @return bool
     */
    protected function canAccessPage($tableName, array $record)
    {
