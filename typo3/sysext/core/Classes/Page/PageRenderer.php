...
class PageRenderer implements \TYPO3\CMS\Core\SingletonInterface
{
     ...
     * @return array
     */
    public function getRequireJsConfig($scope = null)
    {
    ...
    protected function computeRequireJsConfig($isDevelopment, array $loadedExtensions)
    {
        ...
        if (!isset($requireJsConfig['internal']['paths'])) {
            $requireJsConfig['internal']['paths'] = [];
        }
     ...
     */
    protected function getRequireJsLoader()
    {
        $html = '';
     ...
     * @return array
     */
    protected function filterArrayKeys($array, $keys, $keep = true)
    {
        foreach ($array as $key => $value) {
            if (!(in_array($key, $keys, true) === $keep)) {
                unset($array[$key]);
            }
        }
        return $array;
     ...
     * @return string|null
     */
    protected function findRequireJsBaseModuleName($moduleName)
    {
        // trim spaces and slashes & add ending slash
     ...
     */
    protected function addAjaxUrlsToInlineSettings($publicRoutesOnly = false)
    {
        $ajaxUrls = [];
