    ...
    public function fillInFieldArray($table, $id, $fieldArray, $incomingFieldArray, $realPid, $status, $tscPID)
    {
        ...
        if (strstr($id, 'NEW')) {
            ...
            if (is_array($currentRecord) && $GLOBALS['TCA'][$table]['ctrl']['transOrigDiffSourceField'] && $GLOBALS['TCA'][$table]['ctrl']['languageField'] && $currentRecord[$GLOBALS['TCA'][$table]['ctrl']['languageField']] > 0 && $GLOBALS['TCA'][$table]['ctrl']['transOrigPointerField'] && (int)$currentRecord[$GLOBALS['TCA'][$table]['ctrl']['transOrigPointerField']] > 0) {
                ...
                $originalLanguage_diffStorage = Unserialize::unserialize(
        ...
        if (is_array($types_fieldConfig)) {
            foreach ($types_fieldConfig as $vconf) {
                ...
                // Look for transformation flag or defined richtext configuration in TCA:
                if ((string)$incomingFieldArray['_TRANSFORM_' . $vconf['field']] === 'RTE' || !empty($vconf['spec']['richtext'])) {
            ...
            if ($DSelements[$key]['type'] == 'array') {
                ...
                foreach ($dataValues[$key] as $vKey => $data) {
                    if ($callBackFunc) {
                        ...
                        // Look for RTE transformation of field or defined richtext configuration in FlexForm DS:
                        if (($dataValues[$key]['_TRANSFORM_' . $vKey] == 'RTE' || !empty($specConf['richtext'])) && !$this->dontProcessTransformations) {
